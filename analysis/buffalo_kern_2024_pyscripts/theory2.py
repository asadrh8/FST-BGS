...
def Q2_sum_integral_migration(Z, M, m, tmax=1000, thresh=0.01, use_sum=False):
    """
    ...
    for T in end_ts:
        if not asymptoted:
            if use_sum:
                ...
                t = np.arange(T+1)
                assert len(t), T
                integrand = quad(lambda r: np.sum((1-m)**t * (1-r)**t * Z**t)**2, 0, M/2)[0]
            else:
                 if M == 0:
                     # For now, I have only included the computed sum when M = 0, as it is simple enough to compute directly
                     # as opposed to having python do it.
                    integrand = ((1 - Z**(T+1)*(1-m)**(T+1)) / (1 - Z*(1-m)))**2
                 else:
                    integrand = quad(lambda r: ((1-(1-r)**(T+1) * Z**(T+1) * (1-m)**(T+1))/(1 - (1-r)*Z*(1-m)))**2, 0, M/2)[0]
            if last is not None and last > 0:
                if (integrand-last)/last < thresh:
                    asymptoted = True
            else:
...
def bgs_segment_sc16_migration(mu, sh, L, rbp, N, m, asymptotic=True, T_factor=10,
    ...
    Q2 = Q2_sum_integral_migration(Z, M, m, tmax=T_factor*N)
