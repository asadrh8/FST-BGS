...
def minimum_t2(variance):
    ...
    return calculate_t2(Nseff, NUdeff, NR = 0)
...
def approx_g(lam):
    if lam < 0.5:
        ...
        # *sqrt(2*pi*exp(-2))*exp(1.0/log(2*lam)-0.25/lam)
        return 0.5772+log(2*lam)-1.0/4/lam
...
def calculate_t2_m(Ns, NUd, NR, Nm):
    if NR == 0:
        lam = 2*NUd*Ns/((Ns+Nm)*(2*Ns+2*Nm))
        return exp(-lam)+(calculate_g(lam)) / Ns
    else:
        lam = 2*NUd*Ns/((Ns+Nm)*(2*Ns+NR+2*Nm))
        return exp(-lam)+(calculate_g(lam)) / Ns
